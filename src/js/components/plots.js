...
import ScatterPlotWithTrendline from './ScatterPlotWithTrendline';
...
export default class Plots extends React.Component {
    ...
    componentDidMount() {
      ...
      let graph_data = [];
      let elementTraceMap = {}
      this.props.data.allElements.edges.forEach(function(element, index){
      ...
      this.props.data.allMetal.edges.forEach(function(element){
        if(currentState.props.country == element.node.country) {
          ...
          if (element.node.elementTraceAssayId){
            ...
            let elementTraceData = currentState.props.data.allElements.edges[elementTraceMap[elementTraceId]];
            let combined_node = Object.assign(node, elementTraceData.node);
            graph_data.push(combined_node)
          }
        }
      });
      let data = this.convertToWeightPercent(graph_data);
